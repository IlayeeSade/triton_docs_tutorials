    ...
    int OF, int IF, int group_size)
{
    ...
    // Split each accumulator into 2 independent chains for more ILP
    float partial_acc_0_a = 0.0f; // row_idx + 0
    float partial_acc_0_b = 0.0f;
    float partial_acc_1_a = 0.0f; // row_idx + 1
    float partial_acc_1_b = 0.0f;
    float partial_acc_2_a = 0.0f; // row_idx + 2
    float partial_acc_2_b = 0.0f;
    float partial_acc_3_a = 0.0f; // row_idx + 3
    float partial_acc_3_b = 0.0f;
    ...
    for (int k_base = 0; k_base < IF_vec; k_base += BLOCK_DIM_X) {
        ...
        if (k_vec < IF_vec) {
            ...
            for (int step = 0; step < 4; step++) {
                ...
                // Split updates across two independent accumulation chains
                if (step & 1) {
                    partial_acc_0_b += w_deq_0 * act_val;
                    partial_acc_1_b += w_deq_1 * act_val;
                    partial_acc_2_b += w_deq_2 * act_val;
                    partial_acc_3_b += w_deq_3 * act_val;
                } else {
                    partial_acc_0_a += w_deq_0 * act_val;
                    partial_acc_1_a += w_deq_1 * act_val;
                    partial_acc_2_a += w_deq_2 * act_val;
                    partial_acc_3_a += w_deq_3 * act_val;
                }
            }
        }
    }

    // Combine split accumulator chains before reduction
    float partial_acc_0 = partial_acc_0_a + partial_acc_0_b;
    float partial_acc_1 = partial_acc_1_a + partial_acc_1_b;
    float partial_acc_2 = partial_acc_2_a + partial_acc_2_b;
    float partial_acc_3 = partial_acc_3_a + partial_acc_3_b;
